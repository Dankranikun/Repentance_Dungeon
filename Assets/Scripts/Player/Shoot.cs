...
public class Shoot : MonoBehaviour
{
	...
	void Update()
	{
		...
		if (Time.time > shootRateTime) // Solo permite disparar si ha pasado el tiempo
		{
			if (Input.GetKeyDown(KeyCode.UpArrow))
			{
				shootDirection = Vector3.forward; // (0, 0, 1)
			}
			else if (Input.GetKeyDown(KeyCode.DownArrow) && Time.time > shootRateTime)
			{
				shootDirection = Vector3.back; // (0, 0, -1)
			}
			else if (Input.GetKeyDown(KeyCode.LeftArrow) && Time.time > shootRateTime)
			{
				shootDirection = Vector3.left; // (-1, 0, 0)
			}
			else if (Input.GetKeyDown(KeyCode.RightArrow) && Time.time > shootRateTime)
			{
				shootDirection = Vector3.right; // (1, 0, 0)
			}

			// Si se ha presionado alguna tecla, actualizamos el tiempo de disparo
